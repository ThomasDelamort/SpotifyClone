...
const SearchPage = () => {
  const { query, setQuery, search, results, isLoading, error } =
    useSearchStore();
  const { currentSong, isPlaying, playAlbum, togglePlay } = usePlayerStore();

  // debounce: fire the search 300ms after the last keystroke
  useEffect(() => {
        ...
        <div className="p-4 sm:p-6">
          ...
          <div className="relative max-w-xl mb-8">
            ...
            <Input
              ...
              className="h-11 pl-10 bg-zinc-800/60 border-none rounded-full text-base"
            />
          </div>
          ...
          {error && (
            <p className="text-red-400">
              Search failed: {error}. Make sure the backend is running and{" "}
              <code>/api/search</code> is registered.
            </p>
          )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p
                          className={`truncate font-medium ${active ? "text-green-500" : ""}`}
                        >
                          {song.title}
                        </p>
